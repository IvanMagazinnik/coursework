...
К основным преимуществам координатного метода можно отнести: быструю разработку тестов и простоту поиска элементов при неизменности условий. Основными же недостатками этого метода являются:
...
Расспознавание образов в основу этого метода положен метод поиска элементов UI c использованием расспознавания образов и сравнения с образцом. Основными преимуществами данного метода, так же как и в координатном методе является: высокая скорость разработки тестов и простота поиска элементов. К недостаткам данного метода можно отнести:
...
Вышеописанные способы предполагают лишь обнаружение объектов в приложении, но не взаимодействие с ними. Использование же accessibility технологии позволяет управлять объектами приложения. Однако такое взаимодействие будет далеко от реального обращения пользователя с программой.
...
Рассмотрим отдельно pywinauto: бесплатная библиотека для языка python состоящая из набора модулей предназначенных для автоматического гибкого взаимодействия с Microsoft Windows GUI.
...
Работа с мышью в pywinauto была реализованна в модуле `HwndWrapper`. Данный модуль является оберткой для стандартных возможностей win32api по управлению объектами окна. Одной из основных возможностей данного модуля является способность посылать окну приложеия событий мыши, притом передача данных событий реализованна двумя способами: с помощью accessibility технологий в функции `perform_click()` и user like с помощью win32 функций в функции `perform_click_input()`. Также имелись функции предоставляющие удобное обращение к определенным операциям:
...
На Windows все внесенные изменения проверяются с помощью еще одного популярного бесплатного сервиса непрерывной интеграции Appveyor
